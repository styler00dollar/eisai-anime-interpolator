...
bns_test = sorted(
    [str(bn, encoding="utf-8") for bn in dk.bns if bn.startswith(b"test/")]
)
...
for bn in tqdm(bns_test):
    x = dk[bn]
    x = {
        "images": torch.stack([i.tensor() for i in x["images"]], dim=0)[
            None,
        ].to(device),
        "flows": x["flows"][
            None,
            ...
            [
                ...
                (
                    np.mean(
                        [
                            v
                            for bn, v in zip(bns_test, results["lpips"])
                            if "Japan_" in bn
                        ]
                    ),
                    "r:.4E",
            ...
            [
                ...
                (
                    np.mean(
                        [
                            v
                            for bn, v in zip(bns_test, results["chamfer"])
                            if "Japan_" in bn
                        ]
                    ),
                    "r:.4E",
